...
clock = pygame.time.Clock(); fps=10 # frames per second
...
starts = np.random.uniform(0,2*math.pi,len(data))
...
def draw_satellites():
    for i,row in data.iterrows():
        ...
        theta = starts[i] + math.radians(minutes * 360 / period)
        draw_satellite(perigee,apogee,inclination,color,theta)
